...
* 引入Miracle.php
```
include ('miracle/Miracle.php');
$res = (new \Miracle)->index($path)->markdownText($domain)->setMarkdown($path);
```
方法参数说明：
* #### index (string $path[需生成<接口文档>的文件路径])
* #### markdownText (string $domain[域名])
* #### setMarkdown (string $path[md文件保存路径])
